...
export function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 flex w-60 flex-col border-r border-slate-200 bg-white px-4 py-6">
      ...
      {nextEvent ? (
        <Link
          href="/schedule"
          className="mb-6 flex items-center gap-2 rounded-xl bg-brand-50 px-3 py-2.5 transition-colors hover:bg-brand-100"
        >
          ...
          <span className="min-w-0 flex-1 truncate text-xs font-semibold text-brand-700">
            {nextEvent.title}
          </span>
        </Link>
      ) : null}
