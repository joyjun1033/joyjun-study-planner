...
export function HabitAnalysisTable({ stats }: HabitAnalysisTableProps) {
  if (stats.length === 0) {
    return (
      <EmptyState title="아직 등록한 습관이 없습니다" description="위 그리드에서 습관을 추가해 보세요" />
    );
  }
  ...
  return (
    <table className="w-full text-sm">
      <thead>
        <tr className="rounded-lg bg-brand-50 text-left text-xs font-medium text-brand-700 dark:bg-brand-500/10 dark:text-brand-400">
      ...
      <tbody>
        {stats.map((stat) => (
          ...
          >
            <td className="max-w-[8rem] truncate py-3 pl-3 font-medium text-slate-800 dark:text-slate-200">
              {stat.habit.name}
            </td>
            <td className="tnum py-3 text-right text-slate-500 dark:text-slate-400">
              {stat.possible}
            </td>
            <td className="tnum py-3 text-right text-slate-500 dark:text-slate-400">
              {stat.done}
            ...
            <td className="tnum py-3 pr-3 text-right font-semibold text-slate-900 dark:text-slate-100">
              {stat.percent}%
            </td>
          </tr>
        ))}
